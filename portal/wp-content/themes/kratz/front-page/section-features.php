<div class="front_page_section front_page_section_features<?php
	$kratz_scheme = kratz_get_theme_option( 'front_page_features_scheme' );
	if ( ! empty( $kratz_scheme ) && ! kratz_is_inherit( $kratz_scheme ) ) {
		echo ' scheme_' . esc_attr( $kratz_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( kratz_get_theme_option( 'front_page_features_paddings' ) );
?>"
		<?php
		$kratz_css      = '';
		$kratz_bg_image = kratz_get_theme_option( 'front_page_features_bg_image' );
...
<?php
	// Add anchor
	$kratz_anchor_icon = kratz_get_theme_option( 'front_page_features_anchor_icon' );
	$kratz_anchor_text = kratz_get_theme_option( 'front_page_features_anchor_text' );
...
?>
	...
	<?php
	if ( kratz_get_theme_option( 'front_page_features_fullheight' ) ) {
	...
	?>
			...
			<?php
			$kratz_css      = '';
			$kratz_bg_mask  = kratz_get_theme_option( 'front_page_features_bg_mask' );
			$kratz_bg_color_type = kratz_get_theme_option( 'front_page_features_bg_color_type' );
			if ( 'custom' == $kratz_bg_color_type ) {
				$kratz_bg_color = kratz_get_theme_option( 'front_page_features_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_features_content_wrap content_wrap">
			<?php
			// Caption
			$kratz_caption = kratz_get_theme_option( 'front_page_features_caption' );
			if ( ! empty( $kratz_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_features_caption front_page_block_<?php echo ! empty( $kratz_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $kratz_caption, 'kratz_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$kratz_description = kratz_get_theme_option( 'front_page_features_description' );
			if ( ! empty( $kratz_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_features_description front_page_block_<?php echo ! empty( $kratz_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $kratz_description ), 'kratz_kses_content' ); ?></div>
				<?php
			}
			...
			if ( is_active_sidebar( 'front_page_features_widgets' ) ) {
				...
				if ( ! kratz_exists_trx_addons() ) {
					...
					kratz_customizer_need_widgets_message( 'front_page_features_caption', 'ThemeREX Addons - Services' );
				}
			}
			?>
			</div>
