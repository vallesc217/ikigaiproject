...
?>>
		...
		if ( has_post_thumbnail() && $args['featured']=='image') {
			trx_addons_get_template_part('templates/tpl.featured.php',
											...
											apply_filters('trx_addons_filter_args_featured', array(
															...
															'thumb_size' => apply_filters('trx_addons_filter_thumb_size', kratz_get_thumb_size($args['columns'] > 2 ? 'med-services' : 'big'), 'services-light'),
															'no_links' => empty($link),
															'link' => $link,
			...
			?><img src="<?php echo esc_url($meta['image']); ?>" alt="<?php esc_attr_e('Icon', 'kratz'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
			echo !empty($link) 
				? '</a>' 
				: '</span>';
	...
	<div class="sc_services_item_info">
		...
		if (!isset($args['hide_excerpt']) || (int)$args['hide_excerpt']==0) {
			...
			?><div class="sc_services_item_content"><?php echo wp_trim_words(get_the_excerpt(), '17'); ?></div><?php
