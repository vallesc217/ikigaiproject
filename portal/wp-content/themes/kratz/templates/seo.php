...
if ( kratz_is_on( kratz_get_theme_option( 'seo_snippets' ) ) ) {
	...
	<div class="structured_data_snippets">
		...
		<div itemscope itemprop="publisher" itemtype="//schema.org/Organization">
			...
			<?php
			$kratz_logo_image = kratz_get_logo_image();
			if ( ! empty( $kratz_logo_image['logo'] ) ) {
				?>
				<meta itemprop="logo" itemtype="//schema.org/ImageObject" content="<?php echo esc_url( $kratz_logo_image['logo'] ); ?>">
				<?php
			}
			?>
		</div>
		<?php
		if ( kratz_get_theme_option( 'show_author_info' ) != 1 || ! is_single() || is_attachment() || ! get_the_author_meta( 'description' ) ) {
			?>
