...
if ( ! function_exists( 'kratz_hovers_theme_setup3' ) ) {
	add_action( 'after_setup_theme', 'kratz_hovers_theme_setup3', 3 );
	function kratz_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		kratz_storage_set_array_after(
			'options', 'border_radius', array(
				'button_hover' => array(
					'title'   => esc_html__( "Button's hover", 'kratz' ),
					'desc'    => wp_kses_data( __( 'Select hover effect to decorate all theme buttons', 'kratz' ) ),
					'std'     => 'slide_left',
					'options' => array(
						'default'      => esc_html__( 'Fade', 'kratz' ),
						'slide_left'   => esc_html__( 'Slide from Left', 'kratz' ),
						'slide_right'  => esc_html__( 'Slide from Right', 'kratz' ),
						'slide_top'    => esc_html__( 'Slide from Top', 'kratz' ),
						'slide_bottom' => esc_html__( 'Slide from Bottom', 'kratz' ),
					),
					'type'    => 'select',
				),
				'image_hover'  => array(
					'title'    => esc_html__( "Image's hover", 'kratz' ),
					'desc'     => wp_kses_data( __( 'Select hover effect to decorate all theme images', 'kratz' ) ),
					'std'      => 'icon',
					'override' => array(
						'mode'    => 'page',
						'section' => esc_html__( 'Content', 'kratz' ),
					),
					'options'  => kratz_get_list_hovers(),
					'type'     => 'select',
				),
			)
		);
...
if ( ! function_exists( 'kratz_hovers_theme_setup9' ) ) {
	add_action( 'after_setup_theme', 'kratz_hovers_theme_setup9', 9 );
	function kratz_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts', 'kratz_hovers_frontend_scripts', 1010 );
		add_action( 'wp_enqueue_scripts', 'kratz_hovers_frontend_styles', 1100 );
		add_action( 'wp_enqueue_scripts', 'kratz_hovers_responsive_styles', 2000 );
		add_filter( 'kratz_filter_localize_script', 'kratz_hovers_localize_script' );
		add_filter( 'kratz_filter_merge_scripts', 'kratz_hovers_merge_scripts' );
		add_filter( 'kratz_filter_merge_styles', 'kratz_hovers_merge_styles' );
		add_filter( 'kratz_filter_merge_styles_responsive', 'kratz_hovers_merge_styles_responsive' );
		add_filter( 'kratz_filter_get_css', 'kratz_hovers_get_css', 10, 2 );
	}
}

// Enqueue hover styles and scripts
if ( ! function_exists( 'kratz_hovers_frontend_scripts' ) ) {
	function kratz_hovers_frontend_scripts() {
		if ( kratz_is_on( kratz_get_theme_option( 'debug_mode' ) ) ) {
			$kratz_url = kratz_get_file_url( 'theme-specific/theme-hovers/theme-hovers.js' );
			if ( '' != $kratz_url ) {
				wp_enqueue_script( 'kratz-hovers', $kratz_url, array( 'jquery' ), null, true );
			}
		}
	}
}
...
if ( ! function_exists( 'kratz_hovers_frontend_styles' ) ) {
	function kratz_hovers_frontend_styles() {
		if ( kratz_is_on( kratz_get_theme_option( 'debug_mode' ) ) ) {
			$kratz_url = kratz_get_file_url( 'theme-specific/theme-hovers/theme-hovers.css' );
			if ( '' != $kratz_url ) {
				wp_enqueue_style( 'kratz-hovers', $kratz_url, array(), null );
			}
		}
	}
}
...
if ( ! function_exists( 'kratz_hovers_responsive_styles' ) ) {
	function kratz_hovers_responsive_styles() {
		if ( kratz_is_on( kratz_get_theme_option( 'debug_mode' ) ) ) {
			$kratz_url = kratz_get_file_url( 'theme-specific/theme-hovers/theme-hovers-responsive.css' );
			if ( '' != $kratz_url ) {
				wp_enqueue_style( 'kratz-hovers-responsive', $kratz_url, array(), null );
			}
		}
	}
}
...
if ( ! function_exists( 'kratz_hovers_merge_styles' ) ) {
	function kratz_hovers_merge_styles( $list ) {
...
if ( ! function_exists( 'kratz_hovers_merge_styles_responsive' ) ) {
	function kratz_hovers_merge_styles_responsive( $list ) {
...
if ( ! function_exists( 'kratz_hovers_localize_script' ) ) {
	function kratz_hovers_localize_script( $arr ) {
		$arr['button_hover'] = kratz_get_theme_option( 'button_hover' );
		return $arr;
	}
}

// Merge hover effects to the single js
if ( ! function_exists( 'kratz_hovers_merge_scripts' ) ) {
	function kratz_hovers_merge_scripts( $list ) {
...
if ( ! function_exists( 'kratz_hovers_add_icons' ) ) {
	function kratz_hovers_add_icons( $hover, $args = array() ) {
		...
		if ( in_array( $hover, array( 'icons', 'zoom' ) ) ) {
			...
			?>
			<div class="icons">
				<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : $no_link; ?>" <?php kratz_show_layout($target); ?> aria-hidden="true" class="icon-link
									<?php
			...
			// Hover style 'Icon'
			?>
			<div class="icons"><a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : $no_link; ?>" <?php kratz_show_layout($target); ?> aria-hidden="true" class="icon-search-alt"></a></div>
			<?php

		} elseif ( 'dots' == $hover ) {
			// Hover style 'Dots'
			?>
			<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : $no_link; ?>" <?php kratz_show_layout($target); ?> aria-hidden="true" class="icons"><span></span><span></span><span></span></a>
			<?php
			...
			<div class="post_info">
				<div class="post_info_back">
					...
					if ( ! empty( $post_link ) ) {
						?>
							<a href="<?php echo esc_url( $post_link ); ?>" <?php kratz_show_layout($target); ?>>
							<?php
					}
						the_title();
					if ( ! empty( $post_link ) ) {
			...
			// Hover style empty
			?>
			<a href="<?php echo esc_url( $post_link ); ?>" <?php kratz_show_layout($target); ?> aria-hidden="true" class="icons"></a>
			<?php
		}
	}
}
...
// Add styles into CSS
if ( ! function_exists( 'kratz_hovers_get_css' ) ) {
	function kratz_hovers_get_css( $css, $args ) {
...
.sc_button_hover_style_link2.sc_button_hover_slide_left {	background: linear-gradient(to right,	{$colors['alter_link']} 50%, {$colors['alter_dark']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['alter_dark']} !important; }
.sc_button_hover_style_link2.sc_button_hover_slide_right {  background: linear-gradient(to left,	{$colors['alter_link']} 50%, {$colors['alter_dark']} 50%) no-repeat scroll left bottom / 210% 100% {$colors['alter_dark']} !important; }
.sc_button_hover_style_link2.sc_button_hover_slide_top {	background: linear-gradient(to bottom,	{$colors['alter_link']} 50%, {$colors['alter_dark']} 50%) no-repeat scroll right bottom / 100% 210% {$colors['alter_dark']} !important; }
.sc_button_hover_style_link2.sc_button_hover_slide_bottom {	background: linear-gradient(to top,		{$colors['alter_link']} 50%, {$colors['alter_dark']} 50%) no-repeat scroll right top / 100% 210% {$colors['alter_dark']} !important; }


.sc_button_hover_style_link4.sc_button_hover_slide_left {	background: linear-gradient(to right,	{$colors['text_hover4']} 50%, {$colors['text_link4']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['text_link4']} !important; }
.sc_button_hover_style_link4.sc_button_hover_slide_right {  background: linear-gradient(to left,	{$colors['text_hover4']} 50%, {$colors['text_link4']} 50%) no-repeat scroll left bottom / 210% 100% {$colors['text_link4']} !important; }
.sc_button_hover_style_link4.sc_button_hover_slide_top {	background: linear-gradient(to bottom,	{$colors['text_hover4']} 50%, {$colors['text_link4']} 50%) no-repeat scroll right bottom / 100% 210% {$colors['text_link4']} !important; }
.sc_button_hover_style_link4.sc_button_hover_slide_bottom {	background: linear-gradient(to top,		{$colors['text_hover4']} 50%, {$colors['text_link4']} 50%) no-repeat scroll right top / 100% 210% {$colors['text_link4']} !important; }

.sc_button_hover_style_link3.sc_button_hover_slide_left {	background: linear-gradient(to right,	{$colors['text_link']} 50%, {$colors['alter_link']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['alter_link']} !important; }
.sc_button_hover_style_link3.sc_button_hover_slide_right {  background: linear-gradient(to left,	{$colors['text_link']} 50%, {$colors['alter_link']} 50%) no-repeat scroll left bottom / 210% 100% {$colors['alter_link']} !important; }
.sc_button_hover_style_link3.sc_button_hover_slide_top {	background: linear-gradient(to bottom,	{$colors['text_link']} 50%, {$colors['alter_link']} 50%) no-repeat scroll right bottom / 100% 210% {$colors['alter_link']} !important; }
.sc_button_hover_style_link3.sc_button_hover_slide_bottom {	background: linear-gradient(to top,		{$colors['text_link']} 50%, {$colors['alter_link']} 50%) no-repeat scroll right top / 100% 210% {$colors['alter_link']} !important; }


.sc_button_hover_style_link5.sc_button_hover_slide_left {	background: linear-gradient(to right,	{$colors['text_link4']} 50%, {$colors['text_link']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['text_link']} !important; }
.sc_button_hover_style_link5.sc_button_hover_slide_right {  background: linear-gradient(to left,	{$colors['text_link4']} 50%, {$colors['text_link']} 50%) no-repeat scroll left bottom / 210% 100% {$colors['text_link']} !important; }
.sc_button_hover_style_link5.sc_button_hover_slide_top {	background: linear-gradient(to bottom,	{$colors['text_link4']} 50%, {$colors['text_link']} 50%) no-repeat scroll right bottom / 100% 210% {$colors['text_link']} !important; }
.sc_button_hover_style_link5.sc_button_hover_slide_bottom {	background: linear-gradient(to top,		{$colors['text_link4']} 50%, {$colors['text_link']} 50%) no-repeat scroll right top / 100% 210% {$colors['text_link']} !important; }
...
li.active .sc_button_hover_slide_left {		background-position: left bottom !important; color: {$colors['alter_dark']} !important; }
...
.post_featured.hover_icon .icons a:hover:before{
