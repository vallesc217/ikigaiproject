...
while ( have_posts() ) {
    ...
    ?><article id="post-<?php the_ID(); ?>" data-post-id="<?php the_ID(); ?>" <?php post_class('portfolio_page itemscope portfolio_page_details_' . esc_attr($meta['details_position']));
    trx_addons_seo_snippets('', 'Article'); ?>><?php

    do_action('trx_addons_action_article_start', 'portfolio.single');

		// Project details before the content
		if (in_array($meta['details_position'], array('right'))) ob_start();
    if (in_array($meta['details_position'], array('bottom'))){
        // Image
        if ( !trx_addons_sc_layouts_showed('featured') && has_post_thumbnail() && (empty($meta['gallery']) || in_array($meta['gallery_position'], array('none', 'bottom'))) ) {
            ...
            the_post_thumbnail(
                apply_filters('trx_addons_filter_thumb_size', kratz_get_thumb_size( 'portfolio-top'), 'portfolio-single'),
				...
				?><div class="portfolio_page_details"><?php
								...
								if (!empty($item['icon'])) {
									...
									?><span class="portfolio_page_details_item_icon sc_icon_type_<?php echo esc_attr($icon_type); ?> <?php echo esc_attr($icon); ?>"><?php
										if (!empty($svg)) {
											...
											?><img class="sc_icon_as_image" src="<?php echo esc_url($img); ?>" alt="<?php esc_attr_e('Icon', 'kratz'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
										}
									?></span><?php
								}
					...
					if ($trx_addons_output) {
						?><span class="portfolio_page_details_item portfolio_page_details_share"><?php
							// Title
							?><span class="portfolio_page_details_item_title"><?php echo esc_html__('Share', 'kratz'); ?></span><?php
							// Value
		...
		?></section><?php
		if (in_array($meta['details_position'], array('right'))) {
		...
		?><section class="portfolio_page_content_wrap"><?php
							...
							foreach($images as $img) {
								...
								?>">
									<figure><?php
										...
										?><a href="<?php echo esc_url($img); ?>" title="<?php echo esc_attr($img_title); ?>"><img src="<?php echo esc_url($thumb); ?>" alt="<?php esc_attr_e('Gallery item', 'kratz'); ?>"<?php if (!empty($attr[3])) echo ' '.trim($attr[3]); ?>></a><?php
										if (!empty($img_title)) {
			...
			// Image
            if (in_array($meta['details_position'], array('top', 'left', 'right'))) {
                if (!trx_addons_sc_layouts_showed('featured') && has_post_thumbnail() && (empty($meta['gallery']) || in_array($meta['gallery_position'], array('none', 'bottom')))) {
                    ?>
                    <div class="portfolio_page_featured"><?php
                    the_post_thumbnail(
                        apply_filters('trx_addons_filter_thumb_size', kratz_get_thumb_size($meta['details_position'] == 'top' ? 'portfolio-top' : 'portfolio'), 'portfolio-single'),
                        trx_addons_seo_image_params(array(
                            'alt' => the_title_attribute( array( 'echo' => false ) )
                        ))
                    );
