...
jQuery( document ).ready(
	function() {
		...
		kratz_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		if (window.elementor !== undefined) {
			...
			jQuery( '#elementor-panel' )
				.on( 'input change', '[data-setting*="kratz_options_"]', function (e) {
					...
					var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.kratz_post_editor' );
					if ( link.length > 0 ) {
						kratz_change_post_edit_link_elementor( link );
					}
				} )
				.on( 'click', '.kratz_post_editor', function(e) {
					kratz_change_post_edit_link_elementor( jQuery(this) );
					if (jQuery(this).hasClass('kratz_hidden' )) {
						e.preventDefault();
						return false;
					}
				});
		...
		function kratz_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
