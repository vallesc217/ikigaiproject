 ...
 */
function intuitive_customize_register( $wp_customize ) {
	...
	if ( isset( $wp_customize->selective_refresh ) ) {
		$wp_customize->selective_refresh->add_partial( 'blogname', array(
			...
			'render_callback' => 'intuitive_customize_partial_blogname',
		) );
		$wp_customize->selective_refresh->add_partial( 'blogdescription', array(
			'selector' => '.site-description',
			'container_inclusive' => false,
			'render_callback' => 'intuitive_customize_partial_blogdescription',
		) );
	}

	// Important Links.
	$wp_customize->add_section( 'intuitive_important_links', array(
		'priority'      => 999,
		'title'         => esc_html__( 'Important Links', 'intuitive' ),
	) );

	// Has dummy Sanitizaition function as it contains no value to be sanitized.
	intuitive_register_option( $wp_customize, array(
			'name'              => 'intuitive_important_links',
			'sanitize_callback' => 'sanitize_text_field',
			'custom_control'    => 'Intuitive_Important_Links',
			'label'             => esc_html__( 'Important Links', 'intuitive' ),
			'section'           => 'intuitive_important_links',
			'type'              => 'intuitive_important_links',
		)
	);
	// Important Links End.
}
add_action( 'customize_register', 'intuitive_customize_register' );

/**
 * Binds JS handlers to make Theme Customizer preview reload changes asynchronously.
 */
function intuitive_customize_preview_js() {
	wp_enqueue_script( 'intuitive-customize-preview', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'assets/js/customizer.min.js', array( 'customize-preview' ), '20170816', true );
}
add_action( 'customize_preview_init', 'intuitive_customize_preview_js' );
