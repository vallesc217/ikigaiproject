...
?><div id="post-<?php the_ID(); ?>" <?php post_class( 'related_item post_format_' . esc_attr( $kratz_post_format ) ); ?>>
	...
	kratz_show_post_featured(
		array(
			'thumb_size'    => apply_filters( 'kratz_filter_related_thumb_size', kratz_get_thumb_size( (int) kratz_get_theme_option( 'related_posts' ) == 1 ? 'big' : 'med' ) ),
	...
	<div class="post_header entry-header">
		...
		if ( in_array( get_post_type(), array( 'post', 'attachment' ) ) ) {
			?>
			<div class="post_meta">
				<a href="<?php echo esc_url( $kratz_link ); ?>" class="post_meta_item post_date"><?php echo wp_kses_data( kratz_get_date() ); ?></a>
			</div>
			<?php
		}
		?>
