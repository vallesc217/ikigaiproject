...
------------------------------------------------------------------------------- */
if ( ! function_exists( 'kratz_trx_addons_theme_specific_components' ) ) {
	add_filter( 'trx_addons_filter_components_editor', 'kratz_trx_addons_theme_specific_components' );
	function kratz_trx_addons_theme_specific_components( $enable = false ) {
		return false;
	}
}

if ( ! function_exists( 'kratz_trx_addons_theme_specific_setup1' ) ) {
	add_action( 'after_setup_theme', 'kratz_trx_addons_theme_specific_setup1', 1 );
	function kratz_trx_addons_theme_specific_setup1() {
		if ( kratz_exists_trx_addons() ) {
			add_filter( 'trx_addons_cv_enable', 'kratz_trx_addons_cv_enable' );
			add_filter( 'trx_addons_demo_enable', 'kratz_trx_addons_demo_enable' );
			add_filter( 'trx_addons_filter_edd_themes_market', 'kratz_trx_addons_edd_themes_market_enable' );
			add_filter( 'trx_addons_api_list', 'kratz_trx_addons_api_list' );
			add_filter( 'trx_addons_cpt_list', 'kratz_trx_addons_cpt_list' );
			add_filter( 'trx_addons_sc_list', 'kratz_trx_addons_sc_list' );
			add_filter( 'trx_addons_widgets_list', 'kratz_trx_addons_widgets_list' );
		}
	}
}

if ( ! function_exists( 'kratz_trx_addons_theme_specific_setup' ) ) {
	add_action( 'after_setup_theme', 'kratz_trx_addons_theme_specific_setup' );
	function kratz_trx_addons_theme_specific_setup() {
		if ( kratz_exists_trx_addons() ) {
			if ( ! is_admin() ) {
				add_action( 'kratz_action_before_post_meta', 'kratz_trx_addons_action_before_post_meta' );
			}
		}
	}
}
...
// CV
if ( ! function_exists( 'kratz_trx_addons_cv_enable' ) ) {
	function kratz_trx_addons_cv_enable( $enable = false ) {
		// To do: return false if theme not use CV functionality
		return KRATZ_THEME_FREE
					? false     // Free version
					: true;     // Pro version
	}
}
...
// Demo mode
if ( ! function_exists( 'kratz_trx_addons_demo_enable' ) ) {
	function kratz_trx_addons_demo_enable( $enable = false ) {
...
// EDD Themes market
if ( ! function_exists( 'kratz_trx_addons_edd_themes_market_enable' ) ) {
	function kratz_trx_addons_edd_themes_market_enable( $enable = false ) {
...
// API
if ( ! function_exists( 'kratz_trx_addons_api_list' ) ) {
	function kratz_trx_addons_api_list( $list = array() ) {
...
// CPT
if ( ! function_exists( 'kratz_trx_addons_cpt_list' ) ) {
	function kratz_trx_addons_cpt_list( $list = array() ) {
...
// Shortcodes
if ( ! function_exists( 'kratz_trx_addons_sc_list' ) ) {
	function kratz_trx_addons_sc_list( $list = array() ) {
        ...
        $list['blogger']['templates']['default']['posts'] = array(
            'title'  => esc_html__('Posts Default', 'kratz'),
            'layout' => array(
                'featured' => array(
                    'bl' => array(
                        'meta_date'
...
// Widgets
if ( ! function_exists( 'kratz_trx_addons_widgets_list' ) ) {
	function kratz_trx_addons_widgets_list( $list = array() ) {
...
if ( ! function_exists( 'kratz_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'kratz_trx_addons_menu_cache' );
	function kratz_trx_addons_menu_cache( $list = array() ) {
...
if ( ! function_exists( 'kratz_trx_addons_localize_script' ) ) {
	...
	function kratz_trx_addons_localize_script( $arr ) {
		$arr['alter_link_color'] = kratz_get_scheme_color( 'alter_link' );
		return $arr;
	}
}
...
if ( ! function_exists( 'kratz_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'kratz_trx_addons_sc_type', 10, 2 );
	function kratz_trx_addons_sc_type( $list, $sc ) {
		// To do: check shortcode slug and if correct - add new 'key' => 'title' to the list
		if ( 'trx_sc_blogger' == $sc ) {
			$list = kratz_array_merge( $list, kratz_get_list_blog_styles( false, 'sc' ) );
		}
        if ( 'trx_sc_button' == $sc ) {
...
if ( ! function_exists( 'kratz_trx_addons_sc_prepare_atts' ) ) {
	add_filter( 'trx_addons_filter_sc_prepare_atts', 'kratz_trx_addons_sc_prepare_atts', 10, 2 );
	function kratz_trx_addons_sc_prepare_atts( $atts, $sc ) {
		if ( 'trx_sc_blogger' == $sc ) {
			$list = kratz_get_list_blog_styles( false, 'sc' );
			if ( isset( $list[ $atts['type'] ] ) ) {
				$custom_type = '';
				if ( strpos( $atts['type'], 'blog-custom-' ) === 0 ) {
					$blog_id = kratz_get_custom_blog_id( $atts['type'] );
...
if ( ! function_exists( 'kratz_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'kratz_trx_addons_sc_atts', 10, 2 );
	function kratz_trx_addons_sc_atts( $atts, $sc ) {
...
if ( ! function_exists( 'kratz_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'kratz_trx_addons_sc_map', 10, 2 );
	function kratz_trx_addons_sc_map( $params, $sc ) {
		...
		$param = array(
			...
			'heading'          => esc_html__( 'Color style', 'kratz' ),
			'description'      => wp_kses_data( __( 'Select color style to decorate this block', 'kratz' ) ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label'      => true,
			'value'            => array_flip( kratz_get_list_sc_color_styles() ),
			'type'             => 'dropdown',
		);
		...
		if ( 'trx_sc_blogger' == $sc ) {
			...
			$param      = array(
				'param_name'       => 'hover',
				'heading'          => esc_html__( 'Image hover', 'kratz' ),
				'description'      => wp_kses_data( __( 'Select hover effect for the featured image', 'kratz' ) ),
				'edit_field_class' => 'vc_col-sm-4',
				'value'            => array_flip( kratz_get_list_hovers( true ) ),
				'dependency'       => array(
					'element' => 'type',
...
if ( ! function_exists( 'kratz_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'kratz_trx_addons_sc_output', 10, 4 );
	function kratz_trx_addons_sc_output( $output, $sc, $atts, $content ) {
		...
		if ( in_array( $sc, array( 'trx_sc_action' ) ) ) {
			...
			if ( ! empty( $atts['color_style'] ) && ! kratz_is_inherit( $atts['color_style'] ) && 'default' != $atts['color_style'] ) {
...
if ( ! function_exists( 'kratz_trx_addons_sc_item_link_classes' ) ) {
	add_filter( 'trx_addons_filter_sc_item_link_classes', 'kratz_trx_addons_sc_item_link_classes', 10, 3 );
	function kratz_trx_addons_sc_item_link_classes( $class, $sc, $atts=array() ) {
		if ( 'sc_button' == $sc ) {
			if ( ! empty( $atts['color_style'] ) && ! kratz_is_inherit( $atts['color_style'] ) && 'default' != $atts['color_style'] ) {
...
if ( ! function_exists( 'kratz_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'kratz_trx_addons_sc_item_title_tag' );
	function kratz_trx_addons_sc_item_title_tag( $tag = '' ) {
		return 'h1' == $tag ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( ! function_exists( 'kratz_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'kratz_trx_addons_sc_item_button_args', 10, 3 );
	function kratz_trx_addons_sc_item_button_args( $args, $sc, $sc_args ) {
...
if ( ! function_exists( 'kratz_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title', 'kratz_trx_addons_slider_title', 10, 2 );
	function kratz_trx_addons_slider_title( $title, $data ) {
		$title = '';
		if ( ! empty( $data['title'] ) ) {
...
if ( ! function_exists( 'kratz_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles', 'kratz_trx_addons_sc_googlemap_styles' );
	function kratz_trx_addons_sc_googlemap_styles( $list ) {
		$list['dark'] = esc_html__( 'Dark', 'kratz' );
		return $list;
	}
}

// Show reactions in the single posts
if ( ! function_exists( 'kratz_trx_addons_action_before_post_meta' ) ) {
	function kratz_trx_addons_action_before_post_meta() {
		if ( kratz_exists_trx_addons() ) {
...
if ( ! function_exists( 'kratz_trx_addons_tiny_mce_style_formats' ) ) {
	add_filter( 'trx_addons_filter_tiny_mce_style_formats', 'kratz_trx_addons_tiny_mce_style_formats' );
	function kratz_trx_addons_tiny_mce_style_formats( $style_formats ) {
		...
		if ( false && is_array( $style_formats ) && count( $style_formats ) > 0 ) {
			foreach ( $style_formats as $k => $v ) {
				if ( esc_html__( 'List styles', 'kratz' ) == $v['title'] ) {
					$style_formats[ $k ]['items'][] = array(
						'title'    => esc_html__( 'Arrow', 'kratz' ),
						'selector' => 'ul',
...
if ( ! function_exists( 'kratz_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'kratz_filter_allow_override_header_image', 'kratz_trx_addons_allow_override_header_image' );
	function kratz_trx_addons_allow_override_header_image( $allow ) {
		return is_single()
				&& (
					kratz_is_team_page()
					|| kratz_is_cars_page()
					|| kratz_is_cars_agents_page()
					|| kratz_is_properties_agents_page()
					)
				? false
				: $allow;
	}
...
if ( ! function_exists( 'kratz_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size', 'kratz_trx_addons_thumb_size', 10, 2 );
	function kratz_trx_addons_thumb_size( $thumb_size = '', $type = '' ) {
		return $thumb_size;
	}
}
...
if ( ! function_exists( 'kratz_trx_addons_meta_box_fields' ) ) {
	add_filter( 'trx_addons_filter_meta_box_fields', 'kratz_trx_addons_meta_box_fields', 10, 2 );
	function kratz_trx_addons_meta_box_fields( $mb, $post_type ) {
		if ( defined( 'TRX_ADDONS_CPT_TEAM_PT' ) && TRX_ADDONS_CPT_TEAM_PT == $post_type ) {
			if ( ! isset( $mb['email'] ) ) {
				$mb['email'] = array(
					'title'   => esc_html__( 'E-mail', 'kratz' ),
					'desc'    => wp_kses_data( __( "Team member's email", 'kratz' ) ),
					'std'     => '',
					'details' => true,
					'type'    => 'text',
				);
