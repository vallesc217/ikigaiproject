		...
		?>>
			...
			if (!empty($item['image'])) {
				...
				if (!empty($item['image'])) {
					...
					?><div class="sc_price_item_image"><img src="<?php echo esc_url($item['image']); ?>" alt="<?php esc_attr_e('Icon', 'kratz'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>></div><?php
				}
			} else {
			...
			?><div class="sc_price_item_info"><?php
				...
				if (!empty($item['description'])) {
					...
					?><div class="sc_price_item_description"><?php
						foreach ($item['description'] as $str) {
							?><span><?php echo wp_kses(trx_addons_parse_codes($str), 'kratz_kses_content'); ?></span><?php
						}
					?></div><?php
				}
				if (!empty($item['price'])) {
					...
					?><div class="sc_price_item_price"><?php
						if (!empty($item['before_price'])) {
							?><span class="sc_price_item_price_before"><?php echo wp_kses(trx_addons_parse_codes($item['before_price']), 'kratz_kses_content'); ?></span><?php
						}
						?><span class="sc_price_item_price_value"><?php echo wp_kses($parts[0], 'kratz_kses_content'); ?></span><?php
						if (count($parts) > 1 && $parts[1]!='') {
							?><span class="sc_price_item_price_decimals"><?php echo wp_kses($parts[1], 'kratz_kses_content'); ?></span><?php
						}
						if (!empty($item['after_price'])) {
							?><span class="sc_price_item_price_after"><?php echo wp_kses(trx_addons_parse_codes($item['after_price']), 'kratz_kses_content'); ?></span><?php
						}
					?></div><?php
				}
				...
				if (!empty($item['link']) && !empty($item['link_text'])) {
					?><a href="<?php echo esc_url($item['link']); ?>" class="<?php
							echo esc_attr(apply_filters('trx_addons_filter_sc_item_link_classes', 'sc_price_item_link sc_button sc_button_simple', 'sc_price', $args, $item));
					?>"<?php
